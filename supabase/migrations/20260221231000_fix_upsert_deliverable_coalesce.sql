-- Corrige "COALESCE types text and uuid cannot be matched" na RPC upsert_demand_deliverable.
-- O problema era: NULLIF(p_uploaded_by::text, '') retorna TEXT e _uid é UUID; COALESCE(text, uuid) é inválido.
-- Solução: usar apenas COALESCE(p_uploaded_by, _uid) (ambos UUID).
